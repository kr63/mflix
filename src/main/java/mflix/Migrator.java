...
public class Migrator {
    ...
    public static void main(String[] args) {
        ...
        MongoDatabase mflix = MongoClients.create("mongodb://root:example@localhost:27017")
                .getDatabase("mflix");
        MongoCollection<Document> movies = mflix.getCollection("movies");

        Bson dateStringFilter = Filters.and(
                Filters.exists("lastupdated"),
        ...
        for (Document doc : movies.find(dateStringFilter)) {
            ...
            if (updateDate != null) bulkWrites.add(updateDate);
        }

        Bson ratingStringFilter = Filters.and(
                Filters.exists("imdb.rating"),
